...
In addition to the logging and network communications functions, the central Pi also gathers the following data:
  ...
  - Cabin environment (temperature, humidity, pressure, and VOC's) via a BME680 sensor
  - Water tank levels, by measuring the voltage output of existing VDO tank sensors
  - Boat heel angle via a hybrid reading system that queries networked Signal K attitude (e.g., from an NMEA2000 triducer) with an automatic fallback to a local ADXL343 accelerometer if the network source is stale or offline
  - Real-time performance monitoring using a Machine Learning polar model to predict target boat speed based on wind data from SignalK
...
The most significant remote node is a Pi Pico.  The Pico runs a CircuitPython script to gather the following data and transmit it via RFM69HCW radio module to the main Pi for logging:
  ...
  - Engine and exhaust elbow temperatures via DS18B20 temperature sensors
  
At this time two remote sensing options are supported for the refrigerator:
  - An Atmega328P based microprocessor reading from a DS18B20 thermometer, transmitting via RFM69 radio
  - Govee Bluetooth LE thermometers, monitored via a watchdog service on the Pi that reads exported log files
...
The general contents of the project are:
  ...
  - SRC, containing the source files for the Pi (including the automated network switching script), Pico, Atmega328p, and Aft Cabin Pico firmware stability tasks (`pico_todo.md`)
  - Grafana, containing updated multi-year specification files for the Grafana dashboards
